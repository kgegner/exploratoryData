plot6 <- function() {
  ...
  g + geom_point(aes(color=city)) + theme(legend.position="bottom") + facet_grid(city~ EI.Sector,scales="free_y") + labs(title="Annual Emissions for Motor Vehicle Sources", x="Year", y="Emissions (tons)") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
